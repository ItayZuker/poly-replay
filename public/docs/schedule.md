...
- Header range (**Market** / **Live** / **Schedule**): **Market** = all-time confirmed totals for the selected market series; **Live** = since last header reset; **Schedule** = sum of cards on the week grid. **Manual** buys (quote-box overrides) count in **Market** and **Live** only — they do **not** appear on Schedule cards or in the Schedule total. **Trigger Trade** fills (Market Triggers on Trade) count on Schedule cards and the Schedule total whenever the window sits on an active Live placement — **Use Schedule** is not required (that switch only picks the phase setup for Auto Trade). Phase Auto Trade still needs **Use Schedule** on to attribute to cards. Switching ranges shows a spinner on the totals icon while values load.
...
| Triggers (footer) | Market Triggers that are **Trade** + **Active** (read-only list; Edit opens Market). All-time Trade stats. Separate from Replay Triggers. | Replay-only Trigger list (**Add Trigger**, Pause/Active). Stats from last **Run**. |
| Card stats | Phase Auto Trade **and** Trigger Trade outcomes on the active placement (not quote-box manuals) | Filled when you press **Run** (phase + Replay Triggers) |
...
In **Live**, the left-column footer expands above the Live/Replay switcher to show **Triggers** (Trade + Active Market cards with Stop Loss / green / blue / red / P/L). In **Replay**, that Live list hides and the Replay panel expands instead — order top→bottom: **Run**, then **Latency** / **Fill Success** / **Triggers**, then the switcher. A blue border frames the whole screen in Replay.
...
| **Triggers** | Replay-only list of detector triggers (same create/edit dialog as Market **Triggers**). Each card shows **Title**, a **Pause** / **Active** switcher (no Demo/Trade), then a **Stop Loss** exit-reason count and green/blue/red dots (**Success** / held win / **Fail**) with P/L from the last completed **Run** (green already means Take Profit success). **Pause** skips that card on **Run**; **Active** includes it. New cards from **Add Trigger** start **Paused**; older cards without a saved pause flag stay **Active**. Take Profit / Stop Loss are **¢ offsets from the buy fill** (same as Market; **100** disables that exit). Held to window end without TP/SL → **blue** (win) / **red** (loss) on the Trigger card; green is a profitable Take Profit exit. Fees can still make a Take Profit exit Fail. **Add Trigger** opens the shared dialog; definitions are saved per signed-in user in the browser and are separate from Market Triggers. On **Run**, every **Active** listed trigger is applied on each simulated window (with Latency / Fill Success), racing phase Auto Trade the same way (one open position at a time). Frozen for the duration of a **Run**. |
