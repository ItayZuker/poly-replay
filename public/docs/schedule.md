...
- Days × UTC hours — **all 7×24 cells always show** a stats line (green / red / blue dots) and **P/L**, except Replay slots with no usable recordings (missing CLOB and/or Chainlink — see below)
...
| **Replay** | **Idle / reset** (gray = recording count): clean recordings only (no buy/sell markers). **After Run**: last **Run** results when available (Latency / Fill Success / Active Replay Triggers); otherwise re-simulates with current Active triggers — only windows with **both** CLOB book and Chainlink ticks. |
...
Replay uses the same simulation engine as demo Trigger trading. Window times/outcomes come from Mongo (`recorded_windows`), kept in sync with the recorder’s local window JSON (official Gamma payout at finalize, and any outcome backfill). Outcome backfill also stamps Gamma’s settlement close as the Chainlink JSONL tip at window end (mid-window Chainlink samples stay as recorded). The engine needs **both CLOB book and Chainlink tick files** under the recorder’s `DATA_DIR`. On **Live**/Heroku with `SCHEDULE_REPLAY_SERVICE_URL` set, Open Replay ticks are proxied to that recorder. Windows missing either tick stream are excluded from **Run** and from **Replay** Open Replay (idle gray counts and finished all-zero hours show **No Recordings**). **Live** Open Replay still lists every traded window for the hour cell; ticks are shown when present and skipped with ledger markers when not.
