...
| **Profit prediction (¢)** | After trigger: predicted-side Sell (Bid) must reach trigger Buy + this many ¢ for a Prediction Trade sell (1–50, default **5**). Shown when Prediction is On. |
...
| **Right / Wrong** | Sum of **Prediction Trade** outcomes across all cards in the current **Run** (resets when **Run** starts; updates as each card finishes). **Right** = Prediction bought and sold on Profit, or bought and held to settlement and the market won that side. **Wrong** = Prediction bought, not sold, and the market lost that side. Phase Auto Trade fills do **not** count toward Right/Wrong. A window can contribute more than one score when multiple Prediction trades settle. Shown when Prediction is On; stays at 0 when Off. |
