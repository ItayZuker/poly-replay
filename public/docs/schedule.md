...
| **Prediction** | Replay-only detector (same Gap vs UP/DOWN Buy trigger rules as Market **Prediction**). When **On**, also simulates Prediction Trade fills (Buy on trigger, Sell on Profit) using **Order type**, **Shares**, Latency, and Fill Success — PnL is added to the window/card result, and each Prediction trade adds a green/red/blue dot plus Right/Wrong (see trade dots below). **Race with phase Auto Trade** (same as live Prediction Trade): from the start of each window either side may buy first; while one position is open the other cannot buy; after that position sells both may race again. On/Off switch; when **Off**, the footer collapses so only the switcher stays visible and Prediction is **not** run on **Run** or Open Replay; when **On**, the panel expands upward to show the inputs below. Settings and Right/Wrong totals are **not** shared with live Market Prediction. Frozen for the duration of a **Run** (switcher disabled with muted colors). |
| **Order type** | Prediction Trade **Buy** / **Sell** `FAK` or `FOK` (default **FOK**). Shown when Prediction is On. |
| **Shares** | Share count for Prediction Trade buys (1–100000, default **10**). Shown when Prediction is On. |
| **Max Quote (¢)** | Max price of the cheapening Buy when Duration starts (1–99, default **90**). Shown when Prediction is On. |
| **Min Quote (¢)** | Min price of that cheapening Buy when Duration starts (1–99, default **70**). Must be ≤ Max Quote. Shown when Prediction is On. |
...
| **Profit prediction (¢)** | After a Prediction buy fill: predicted-side Sell (Bid) must reach **fill price** + this many ¢ for a Prediction Trade sell / Right (1–50, default **5**). Shown when Prediction is On. |
...
| **Right / Wrong** | Sum of **Prediction Trade** outcomes across all cards in the current **Run** (resets when **Run** starts; updates as each card finishes). **Right** = Prediction bought and sold when Bid reached **fill + Profit prediction**, or bought and held to settlement and the market won that side. **Wrong** = Prediction bought, not sold on that Profit target, and the market lost that side. Phase Auto Trade fills do **not** count toward Right/Wrong. A window can contribute more than one score when multiple Prediction trades settle. Shown when Prediction is On; stays at 0 when Off. |

It sends the placed cards (and their setups) for the areas they cover on the week grid. For each card it loads recorded ticks in that day/hour slot and runs the same simulation engine as demo trading. Cards are processed **top-left first** (Monday → Sunday, then earlier UTC hour). Results stream back **one card at a time** (green / red / blue / gray + PnL, plus Prediction Right/Wrong for that card).
...
| Trade | Dot |
|-------|-----|
| Sold for profit (phase or Prediction Profit sell) | **Green** |
| Sold for loss (phase) | **Red** |
| Held to settlement and market won that side | **Blue** |
| Held to settlement and market lost that side | **Red** |
| Prediction sold on Profit | **Green** (also **Right**) |
| Prediction held, market won | **Blue** (also **Right**) |
| Prediction held, market lost | **Red** (also **Wrong**) |
