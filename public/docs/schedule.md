...
After a **Run**, cards that have tick data show **Open** in the card ⋮ menu. **Open** launches the **Open Replay** popup for that card’s windows: a scrollable window list (outcome + PnL), graph playback with scrubber/speed, and a hits view. Above the graph (graph view only), **PTB**, **Current**, and **Gap** boxes (equal width) update with the scrubber/playhead from Chainlink ticks. On the graph, the price line uses **Chainlink** ticks only (no book-price fallback), anchors the end of the window to the **official recorded close**, and colors green/red from the stored market **up/down** outcome (same source as Settlement) — not from the last live tick vs PTB alone. The price line and buy/sell hits reveal up to the scrubber (drag to the end of the window for the full line and all hits). The popup shows the **same** windows and fills as the card stats from that run (not a fresh random re-roll of Fill success). After a server restart, press **Run** again before Open so hits match the card.
