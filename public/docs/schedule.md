...
| **Profit prediction (¢)** | After trigger: predicted-side Sell (Bid) must reach trigger Buy + this many ¢ before window end for **Right** (1–50, default **5**). Window outcome is ignored. Shown when Prediction is On. |
...
| **Right / Wrong** | Sum of scored predictions across all cards in the current **Run** (resets when **Run** starts; updates as each card finishes). A window can contribute more than one Right if Profit prediction hits and the detector re-triggers later in Trigger Area. Shown when Prediction is On; stays at 0 when Off. |
