...
On the **Wallets** card, an open icon (top-right of the title row) replaces the heatmap week grid with trader wallets recorded for the **currently selected market** (address, sightings, plus **I WON** / **I LOST** for *your* settled trades in windows where that wallet was present). Each address links to that trader’s Polymarket profile (`polymarket.com/profile/0x…`) in a new tab. Default list is the **top 100 by sightings** (most → least). Click **Sightings**, **I WON**, or **I LOST** to re-fetch the top 100 for that ranking (high→low, then low→high); a spinner appears left of the label while loading. Click the open icon again to return to the heatmap. Changing the market while the list is open reloads it. Win/loss counts need per-window trader lists (stored going forward with recording retention); older windows without that list do not contribute.
