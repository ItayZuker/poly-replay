...
| **Prediction** | Manipulation / prediction detector. Per-series switch; the label shows **· On** / **· Off**. Detects adverse UP/DOWN Buy quotes vs Gap (visual flag only — does not place or cancel orders). **Settings** (On/Off, Max Quote, Shift, Duration, Trigger Area) can be changed from any host and sync per series; **triggers and scoring run only on the deployed app** (not localhost), so local + Heroku do not double-count. **Max Quote (¢)**, **Shift (¢)**, **Duration (Sec)**, and **Trigger Area** stay visible in the same control; they are disabled (muted) when Off. Active window span between the handles is shown in blue |
